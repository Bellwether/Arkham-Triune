...
function bootController(app, file) {
  ...
  var singularNameException = controllerName === 'facebook';
  var controllerRouteName = singularNameException ? controllerName : controllerName+'s'
  var prefix = controllerName === 'app' ? '/' : ('/'+controllerRouteName);
...
function routeAction(app, action, prefix, callback) {
  switch(action) {
    ...
    case 'destroy':
      ...
      break;
    default:
      var defaultRoute = prefix === '/' ? '/' + action : prefix + '/' + action;
      app.get(defaultRoute, callback);
      break;
  }	
}
