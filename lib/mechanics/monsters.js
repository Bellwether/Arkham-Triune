...
console.log('trapMonster '+index+' '+map.tileAt(index).trapableMonster)
...
console.log("TRAP MATCHES "+JSON.stringify(matches))
  ...
  enchant: function(map, turn) {
    ...
    //   if (map.tileAt)
    //   if (!hasExit(i, cells)) {
    //     console.log('does not have exit for monster on idx '+i+' '+hasExit(i, cells))
    //     monsters.trapMonster(map, turn, i);
    //   };
    // });
  },
    ...
    indices.forEach(function(index) {
      if (map.tileAt(index).transportableMonster) {
        if (map.cellAt(index).neighbors.some(isSummoningCircle)) {
          var path = monsters.moveToRandom(map, map.emptyCells(), index);
